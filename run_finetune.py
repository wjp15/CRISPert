...
MODEL_CLASSES = {
    ...
    "dnaextend10": (BertConfig, BertForSequenceClassification, DNATokenizer),
...
def train(args, train_dataset, model, tokenizer):
    ...
    for i in range(len(train_dataset)):
        if train_dataset[i][3].item() == 0:
            #train_weights.append((1/109854)*0.7)         # for a ~ 70/30 class split
            train_weights.append(1/(54488))             # correct weights for a 50/50 split on the training data 
            #train_weights.append(1/(122061-525))          # old weights that give an almost 50/50 split, kept for consistancy when comparing vs old runs
        else:
            #train_weights.append((1/472)*0.3)
            train_weights.append(1/371)
    ...
    train_sampler = WeightedRandomSampler(weights=train_weights,num_samples=args.n_samples_dataset,replacement=True)
    #train_sampler = SequentialSampler(train_dataset)
    print("--------------------------------------------------------")
    print("Sampling with weighted random samples to balance dataset")
    print("--------------------------------------------------------")
    #print(train_dataset)
    ...
    #print("If this prints, remote editing with vs code ssh is working")  # code added
    global_step = 0
    epochs_trained = 0
    ...
    if os.path.exists(args.model_name_or_path):
        ...
        try:
            ...
            global_step = 0 # new line to overide previous line since i was using pretrain checkpoints as models
        except:
            global_step = 0
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.task_name == "dnacrispr":
                #inputs = {"input_ids": batch[0], "attention_mask": batch[1], "labels": batch[3], "feature_a_ids": batch[4], "feature_b_ids": batch[5], "feature_c_ids": batch[6], "feature_d_ids": batch[7]}
                ...
                #print(inputs)    
            if args.task_name == "dnacrispr2":
                inputs = {"input_ids": batch[0], "attention_mask": batch[1], "labels": batch[3],"features_comb": batch[4]}
                #print("###########################")
                #print(inputs["features_comb"].shape)
            if args.task_name == "dnaextend10":
                inputs = {"input_ids": batch[0], "attention_mask": batch[1], "labels": batch[3],"input_ids2": batch[4], "attention_mask2": batch[5]}
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    aupr = average_precision_score(out_label_ids, probs)
                    ...
                    output_train_file = os.path.join(train_output_dir, prefix, "train_results.txt")
                    with open(output_train_file, "a") as writer:

                        train_result = ""

                        writer.write(train_result + str(loss_scalar)[:5] + " " + str(aupr)[:5] +"\n")
...
def evaluate(args, model, tokenizer, prefix="", evaluate=True):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        with open(output_eval_file, "a") as writer:
            ...
            writer.write(eval_result + str(eval_loss)[:5] +"\n")
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    if task == "dnacrispr":
        #print("############load and cache pre tensor##############")
        #print([f.feature_a_ids for f in features])
        ...
        #all_feature_c_ids = torch.tensor([f.feature_c_ids for f in features], dtype=torch.long)
        #all_feature_d_ids = torch.tensor([f.feature_d_ids for f in features], dtype=torch.long)
        #print("############load and cache##############")
        #print(all_feature_a_ids)
        #dataset = TensorDataset(all_input_ids, all_attention_mask, all_token_type_ids, all_labels, all_feature_a_ids)
        ...
        all_features_comb = torch.tensor([f.features_comb for f in features], dtype=torch.long)
        #all_features_comb = torch.tensor([f.features_comb for f in features], dtype=torch.float64) #changed for RNN
        dataset = TensorDataset(all_input_ids, all_attention_mask, all_token_type_ids, all_labels, all_features_comb)
        ...
        dataset = TensorDataset(all_input_ids, all_attention_mask, all_token_type_ids, all_labels, all_input_ids2, all_attention_mask2, all_token_type_ids2)
    else:
...
def main():
    ...
    parser.add_argument(
        ...
        default=15000,
        type=int,
        help="The number of samples taken with weightedrandomsampler from the dataset. "
        "The larger the number here, the more the minority class will be oversampled and more unique samples will be seen from the majority class.",
    )    
    ...
    if not args.do_visualize and not args.do_ensemble_pred:
        ...
        #print("#############################")
        #print(model.feature_classifier.weight)
        #print("#############################")
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0) and args.task_name not in [ "dna690","dnacrispr","dnacrispr2","dnaprom","dnaextend10"]:
        print("----------------------------------test--------------------------------------")
