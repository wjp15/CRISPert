...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        #self.pos_tag_embeddings = nn.Embedding(2, 768)
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None):
        ...
        #embeddings = inputs_embeds + position_embeddings + token_type_embeddings + pos_tag_embeddings
...
class BertEmbeddingsFeatures(nn.Module):
    """Construct the embeddings from word, position and token_type embeddings.
    """
    ...
    def __init__(self, config):
        ...
        #self.feature_embeddings = nn.Embedding(2, 768)
        self.feature_a_embeddings = nn.Embedding(250, 768)
        self.feature_b_embeddings = nn.Embedding(250, 768)
        #self.feature_c_embeddings = nn.Embedding(2, 768)
        #self.feature_d_embeddings = nn.Embedding(2, 768)
    ...
    def forward(self, input_ids=None, feature_a_ids=None, feature_b_ids=None, feature_c_ids=None, feature_d_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None):
    #def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None):
        if input_ids is not None:
            input_shape = input_ids.size()
        else:
        ...
        position_embeddings = self.position_embeddings(position_ids)
        token_type_embeddings = self.token_type_embeddings(token_type_ids)
        # print(feature_a_ids[0])
        # print("#########################here######################")
        feature_a_embeddings = self.feature_a_embeddings(feature_a_ids)
        feature_b_embeddings = self.feature_b_embeddings(feature_b_ids)
        #feature_c_embeddings = self.feature_c_embeddings(feature_c_ids)
        #feature_d_embeddings = self.feature_d_embeddings(feature_d_ids)
        
        embeddings = inputs_embeds + position_embeddings + token_type_embeddings + feature_a_embeddings + feature_b_embeddings
        #embeddings = inputs_embeds + position_embeddings + token_type_embeddings
        embeddings = self.LayerNorm(embeddings)
        embeddings = self.dropout(embeddings)
        return embeddings
...
class BertModel(BertPreTrainedModel):
    ...
    Examples::
        ...
        if token_type_ids is None:
            token_type_ids = torch.zeros(input_shape, dtype=torch.long, device=device)
#        if pos_tag_ids is None:
#            pos_tag_ids = torch.zeros(input_shape, dtype=torch.long, device=device)            
...
#            head_mask = [None] * self.config.num_hidden_layers
        ...
        embedding_output = self.embeddings(
            input_ids=input_ids, position_ids=position_ids, token_type_ids=token_type_ids, inputs_embeds=inputs_embeds #, pos_tag_ids=pos_tag_ids
        )
...
class BertModelFeatures(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.embeddings = BertEmbeddingsFeatures(config)
        self.encoder = BertEncoder(config)
        self.pooler = BertPooler(config)
    ...
    def forward(
        ...
        embedding_output = self.embeddings(
            input_ids=input_ids, position_ids=position_ids, token_type_ids=token_type_ids, inputs_embeds=inputs_embeds, feature_a_ids=feature_a_ids,
            feature_b_ids=feature_b_ids,  feature_c_ids=feature_c_ids,  feature_d_ids=feature_d_ids
        )
        #print("-----embedding_output---------")
        #print(embedding_output.shape)
...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        #self.extra_layer = nn.Linear(184,config.hidden_size)
        # self.extra_layer_a = nn.Linear(52,config.hidden_size)
        # self.extra_layer_b = nn.Linear(52,config.hidden_size)
        # self.extra_layer_c = nn.Linear(52,config.hidden_size)
        # self.extra_layer_d = nn.Linear(52,config.hidden_size)
        self.classifier = nn.Linear(config.hidden_size, self.config.num_labels)
        #self.feature_classifier = nn.Linear(config.hidden_size +184, self.config.num_labels)
       #self.feature_classifier = nn.Linear(config.hidden_size+768, self.config.num_labels)
        # self.conv1 = nn.Conv2d(config.hidden_size,, 3)
    
        #self.class_weights = torch.tensor(config.class_weights, dtype=torch.float32)

        #self.rnn = nn.LSTM(input_size=4,hidden_size=100, bidirectional=True, num_layers=1, batch_first=True)
    ...
    # 2nd bert model taking on the potential off target
        # outputs2 = self.bert2(
        #     input_ids2,
...
#    #combine outputs with first bert model before then running them through the classifier 

#         pooled_output = pooled_output + pooled_output2
    #################2nd bert ends here ####################################
        #pooled_output = self.extra_layer(pooled_output)   # extra layer added to increase complexity
        #pooled_output = self.extra_layer(pooled_output)
        #pooled_output = self.extra_layer(pooled_output)
        #pooled_output = self.extra_layer(pooled_output) 
        #pooled_output = self.extra_layer(pooled_output)
        ############# RNN #################
        #features_comb= features_comb.transpose(1, 2)   # change dimensions for RNN
        #features_comb=features_comb.to(torch.float32)  #change datatype for RNN
        #print(features_comb.shape)
        #rnn_outputs, (ht, ct)= self.rnn(features_comb)
        #print(rnn_outputs.shape)
        #print(pooled_output.shape)
        #print(ht.shape)
        #print(ht.squeeze(0).sum(dim=0).shape)
        #print(ht.shape)
        #print("###################")
        #pooled_output_features_rnn = torch.cat((pooled_output,ht.squeeze(0).sum(dim=0)), dim=-1)
        #print(pooled_output_features_rnn.shape)
        #logits = self.feature_classifier(pooled_output_features_rnn)
        #########################################
       
        # print("#######CNN#######")
        # print(outputs[0].shape)
        # print(feature_a_ids.shape)
        # seq = outputs[0]
        # feature_a_ids = feature_a_ids.float()
        # feature_b_ids = feature_b_ids.float()
        # feature_c_ids = feature_c_ids.float()
        # feature_d_ids = feature_d_ids.float()

        # feature_a_ids = self.extra_layer_a(feature_a_ids)
        # feature_b_ids = self.extra_layer_b(feature_b_ids)
        # feature_c_ids = self.extra_layer_c(feature_c_ids)
        # feature_d_ids = self.extra_layer_d(feature_d_ids)
        ...
        #print(features_comb.dtype)

        #features_comb = features_comb.float()
        #print(features_comb.dtype)

        #features_comb = self.extra_layer(features_comb)
        #print(features_comb.shape)
        #pooled_output_features = pooled_output + features_comb

        #pooled_output_features = torch.cat((pooled_output, features_comb), dim=-1)

        #print(pooled_output_features.shape)
        #print(pooled_output_features.dtype)

        #logits = self.feature_classifier(pooled_output_features)



        logits = self.classifier(pooled_output)
        #logits = self.classifier(pooled_output_features)
...
class BertForSequenceClassificationFeatures(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels

        self.bert = BertModelFeatures(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)

        self.extra_layer = nn.Linear(config.hidden_size,config.hidden_size)
        self.extra_layer2 = nn.Linear(config.hidden_size,339)

        self.classifier = nn.Linear(config.hidden_size, self.config.num_labels)
        self.classifier2 = nn.Linear(339,2)
    ...
    def forward(
        ...
        outputs = self.bert(
            ...
            token_type_ids=token_type_ids,
            position_ids=position_ids,
            feature_a_ids=feature_a_ids,
            feature_b_ids=feature_b_ids,
            feature_c_ids=feature_c_ids,
            feature_d_ids=feature_d_ids,
            head_mask=head_mask,
            inputs_embeds=inputs_embeds,
        )
        ...
        #pooled_output = self.extra_layer(pooled_output)
        #pooled_output = self.extra_layer2(pooled_output)
        #pooled_output = self.extra_layer(pooled_output)
        #pooled_output = self.extra_layer(pooled_output)
        ...
        if labels is not None:
            if self.num_labels == 1:
                ...
                #print("###################################################")
                #m = nn.Sigmoid()
                #new = m(logits.view(-1, self.num_labels))
                #new2 = torch.argmax(new)
                #loss = wasserstein_distance(new2,labels.view(-1))
                #print("###################################################")
            outputs = (loss,) + outputs

        return outputs  # (loss), logits, (hidden_states), (attentions)

class BertForSequenceClassificationFeatures2(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        ...
        self.feature_classifier = nn.Linear(config.hidden_size +184, self.config.num_labels)
        #self.feature_classifier_biLinear = nn.Bilinear(config.hidden_size,184,2)
    ...
    def forward(
        ...
        pooled_output_features = torch.cat((pooled_output, features_comb), dim=-1)
        #pooled_output_features = torch.cat((cls_embedding, features_comb), dim=-1)


        logits = self.feature_classifier(pooled_output_features)
        #logits = self.feature_classifier_biLinear(pooled_output,features_comb.float())
        ...
        #weights=torch.FloatTensor([1,125]).cuda()
        if labels is not None:
            if self.num_labels == 1:
                #  We are doing regression
                loss_fct = MSELoss()
